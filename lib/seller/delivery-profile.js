...
import { getShippingZonePricingBasis } from "@/lib/seller/shipping-weight-requirements";
...
function normalizeDirectDelivery(directDelivery = {}, source = {}) {
  const explicitRules = Array.isArray(directDelivery.pricingRules)
    ? directDelivery.pricingRules
        .map((rule, index) => normalizePricingRule(rule, index))
        .filter((rule) => rule.isActive)
        .slice(0, 1)
        .map((rule, index) => ({
          ...rule,
          id: rule.id || `pricing-${index + 1}`,
          label: rule.label || "Direct delivery",
          minDistanceKm: null,
          maxDistanceKm: null,
          minOrderValue: null,
          maxOrderValue: null,
...
function normalizeShippingZone(zone = {}, index = 0) {
  const baseRule =
    Array.isArray(zone.pricingRules) && zone.pricingRules.length
      ? normalizePricingRule(zone.pricingRules[0], 0)
      : normalizePricingRule(
          {
            id: `${toStr(zone.id || `zone-${index + 1}`)}-standard`,
            label: toStr(zone.country || zone.label || zone.name || `Shipping zone ${index + 1}`),
            fee: zone.fee ?? 0,
          },
          0,
        );
  const pricingRules = baseRule && baseRule.isActive
    ? [
        {
          ...baseRule,
          minDistanceKm: null,
  ...
  return {
    ...
    rateMode: "flat",
    pricingBasis: getShippingZonePricingBasis(zone),
    courierKey: "",
    courierServiceLabel: "",
    pricingRules,
    isFallback: false,
...
function calculateShippingZoneAmount(zone, matchedRule, subtotalIncl, shipmentSummary) {
  const freeThreshold = typeof matchedRule.freeAboveOrderValue === "number" ? matchedRule.freeAboveOrderValue : null;
  if (freeThreshold != null && subtotalIncl >= freeThreshold) return 0;

  const fee = sanitizeMoney(matchedRule.fee);
  const pricingBasis = getShippingZonePricingBasis(zone);
  if (pricingBasis === "per_item") {
    const count = Math.max(1, Number(shipmentSummary?.parcelCount || 0));
    return sanitizeMoney(fee * count);
  }
  if (pricingBasis === "per_kg") {
    const weight = Math.max(0, Number(shipmentSummary?.billableWeightKg || shipmentSummary?.actualWeightKg || 0));
    if (weight <= 0) return null;
    return sanitizeMoney(fee * weight);
  }
  return fee;
}
...
export function resolveSellerDeliveryOption({
  ...
  const shipmentSummary = summarizeShipmentParcels(Array.isArray(parcels) ? parcels : []);
  ...
  if (normalized.directDelivery.enabled) {
    ...
    if (sameCountry && sameArea && radiusAllows) {
      ...
      unavailableReasons.push("No direct-delivery pricing rule matches this order value or distance.");
    } else {
      if (!sameCountry) unavailableReasons.push("This seller only delivers within the same country as their shipping origin.");
      if (typeof shopperDistanceKm !== "number") {
        unavailableReasons.push("We could not confirm the delivery distance for this address.");
      } else if (normalized.directDelivery.radiusKm > 0 && shopperDistanceKm > normalized.directDelivery.radiusKm) {
        unavailableReasons.push(
          `This address is ${Math.round(shopperDistanceKm)}km away, outside the seller's ${normalized.directDelivery.radiusKm}km direct-delivery radius.`,
        );
      } else if (!sameArea) {
        unavailableReasons.push("This address does not match the seller's direct-delivery area.");
      }
    }
  }
  ...
  for (const zone of normalized.shippingZones) {
    ...
    const amountIncl = calculateShippingZoneAmount(zone, matchedRule, subtotalIncl, shipmentSummary);
    if (amountIncl == null) {
      unavailableReasons.push("This seller uses per-kg shipping for this country, but one or more variants are missing weight.");
      continue;
    }
    const pricingBasis = getShippingZonePricingBasis(zone);
    return {
