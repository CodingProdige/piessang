...
export async function processSellerDeliverySettingsReminders() {
  const db = getAdminDb();
  if (!db) {
  ...
  for (const [sellerKey, hiddenCount] of hiddenCounts.entries()) {
    ...
    if (sellerDeliverySettingsReady(seller?.deliveryProfile || {})) continue;

    const reminders = seller?.deliverySettingsReminders && typeof seller.deliverySettingsReminders === "object"
