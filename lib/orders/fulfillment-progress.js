...
export function buildOrderDeliveryProgress(order) {
  ...
  const totals = items.reduce(
    (acc, item) => {
      ...
      const cancelled = toLower(item?.fulfillment_tracking?.status) === "cancelled";
      ...
      acc.progressUnits += quantity * (closed ? 100 : progressPercent);
      if (delivered) {
        acc.deliveredLines += 1;
        acc.deliveredUnits += quantity;
      }
      if (cancelled) {
        acc.cancelledLines += 1;
        acc.cancelledUnits += quantity;
      }
      if (closed) {
        acc.closedLines += 1;
        acc.closedUnits += quantity;
      } else {
        acc.pendingLines += 1;
  ...
  return {
    ...
    progress: {
      ...
      isComplete: totals.totalLines > 0 && totals.closedLines === totals.totalLines,
    },
  };
}
