...
function getOrderStatus(order) {
  return toLower(order?.order?.status?.order || "");
}

function getOrderFulfillmentStatus(order) {
  return toLower(order?.order?.status?.fulfillment || "");
}
...
function normalizeStatus(rawStatus, order) {
  ...
  if (["not_started", "draft"].includes(status)) return "not_started";
...
export function deriveAggregateOrderStatuses(items, order = null) {
  ...
  if (!activeItems.length) {
    return {
      orderStatus: getOrderStatus(order) || "draft",
