...
export function buildPlatformOrderDocument({
  ...
  const shippingBreakdown = Array.isArray(deliveryFeeData?.shippingBreakdown) ? deliveryFeeData.shippingBreakdown : [];
  const shippingBaseTotal = r2(totals?.shippingBaseTotal || 0);
  const shippingMarginTotal = r2(totals?.shippingMarginTotal || totals?.shippingMarkupTotal || 0);
  const shippingFinalTotal = r2(totals?.shippingFinalTotal || deliveryFeeData?.sellerAmountIncl || 0);
  const platformDeliveryIncl = 0;
  const sellerDeliveryIncl = shippingFinalTotal;
  ...
  return {
    ...
    pricing_snapshot: {
      ...
      sellerDeliveryIncl,
      deliveryTotalIncl: shippingFinalTotal,
      shippingBaseTotal,
      shippingMarginTotal,
      shippingFinalTotal,
    ...
    totals: {
      ...
      shippingMarginTotal,
      shippingFinalTotal,
      grandTotal: r2(totals?.grandTotal || finalPayableIncl),
