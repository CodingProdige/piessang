...
function r2(value) {
  ...
  return Number.isFinite(parsed) ? normalizeMoneyAmount(parsed) : 0;
}
...
export function normalizeReturnOwnerForLine(order = {}, item = {}) {
  ...
  const shippingEntry = getSellerShippingEntry(order, seller.sellerCode, seller.sellerSlug);
  ...
  if (fulfillmentOwner === "platform") {
    return {
      ...
      seller,
      deliveryType: "platform_fulfillment",
    };
  }
  ...
  return {
    ...
    responsibility: "The seller manages shipping-related returns for this item.",
    seller,
    deliveryType: shippingEntry?.legacy ? "legacy_shipping" : "shipping",
  };
}
