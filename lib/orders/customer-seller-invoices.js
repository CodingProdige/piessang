...
const PIESSANG_LOGO_URL = "https://piessang.co.za/logo/Piessang%20Logo%20Full%20-%20Clipped.png";
...
export function buildCustomerSellerInvoicePayload({
  ...
  return {
    seller: {
      ...
      logoUrl: toStr(sellerBusiness?.logoUrl),
    },
    platform: {
  ...
  return `
    ...
    <html lang="en">
      <head>
        ...
        <style>
          * { box-sizing: border-box; }
          ...
          .header { display:flex; justify-content:space-between; gap:20px; align-items:flex-start; border-bottom:2px solid #111; padding-bottom:16px; margin-bottom:16px; }
          .brand { display:flex; gap:18px; align-items:flex-start; }
          .logo { width:170px; max-height:58px; object-fit:contain; object-position:left center; }
      ...
      <body>
        ...
        <div class="header">
          <div class="brand">
            ${payload?.platform?.logoUrl ? `<img src="${esc(payload.platform.logoUrl)}" alt="Piessang" class="logo" />` : ""}
            <div class="company">
