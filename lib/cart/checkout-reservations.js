...
export const VARIANT_CHECKOUT_RESERVATION_TTL_MS = 60 * 1000;
const ACTIVE_RESERVATION_CACHE_TTL_MS = 10 * 1000;
const activeReservationCache = new Map();
  ...
  const result = await db.runTransaction(async (tx) => {
    ...
    const expiresAt =
      Number.isFinite(requestedExpiresAtMs) && requestedExpiresAtMs > Date.now()
        ...
        : new Date(Date.now() + VARIANT_CHECKOUT_RESERVATION_TTL_MS).toISOString();
...
export async function loadActiveCheckoutReservationMap({ productId, variantIds }) {
  ...
  const cacheKey = `${productKey}:${Array.from(needles).sort().join(",")}`;
  const cached = activeReservationCache.get(cacheKey);
  if (cached && Date.now() - cached.createdAt < ACTIVE_RESERVATION_CACHE_TTL_MS) {
    return new Map(cached.value);
  }

  const refs = Array.from(needles).map((variantId) =>
    db.collection(VARIANT_CHECKOUT_RESERVATIONS_COLLECTION).doc(buildReservationDocId(productKey, variantId)),
  );
  const docs = [];
  ...
  activeReservationCache.set(cacheKey, { createdAt: Date.now(), value: Array.from(map.entries()) });
  return map;
}
...
export async function reclaimExpiredVariantCheckoutReservations() {
  ...
  if (reclaimed.length) activeReservationCache.clear();
  return reclaimed;
}
