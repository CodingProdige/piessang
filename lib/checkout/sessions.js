...
const CHECKOUT_SESSION_RESERVATION_TTL_MS = 60 * 1000;

function reservationExpiresAtFrom(createdAt) {
  ...
  return new Date(baseMs + CHECKOUT_SESSION_RESERVATION_TTL_MS).toISOString();
}

async function loadCartReservationExpiresAt(cartOwnerId) {
  const db = getAdminDb();
  const safeCartOwnerId = toStr(cartOwnerId);
  if (!db || !safeCartOwnerId) return "";
  const snap = await db.collection("carts").doc(safeCartOwnerId).get().catch(() => null);
  if (!snap?.exists) return "";
  const value = toStr(snap.data()?.cart?.checkout_reservation_expires_at);
  const valueMs = value ? new Date(value).getTime() : 0;
  return Number.isFinite(valueMs) && valueMs > 0 ? new Date(valueMs).toISOString() : "";
}

function isReservationExpired(session) {
  const expiresAtMs = new Date(session?.reservationExpiresAt || reservationExpiresAtFrom(session?.createdAt)).getTime();
...
export async function ensureCheckoutSession({ cartOwnerId, cartId }) {
  ...
  const timestamp = nowIso();
  const cartReservationExpiresAt = await loadCartReservationExpiresAt(safeCartOwnerId);
    ...
    if (existingSessionId) {
      ...
      if (sessionSnap.exists) {
        ...
        if (
          ...
          && toStr(session?.status || "active") === "expired"
        ) {
          const reservationExpiresAt = cartReservationExpiresAt || toStr(session?.reservationExpiresAt) || reservationExpiresAtFrom(session?.createdAt);
          await Promise.all([
            mappingRef.set({ status: "expired", reservationExpiresAt, updatedAt: timestamp }, { merge: true }),
            upsertCartLifecycle({ cartOwnerId: safeCartOwnerId, cartId: safeCartId, status: "active", extra: { checkoutSessionId: existingSessionId, reservationStatus: "expired" } }),
          ]);
          return {
        ...
        if (
          ...
          const reservationExpiresAt = cartReservationExpiresAt || toStr(session?.reservationExpiresAt) || reservationExpiresAtFrom(session?.createdAt);
          if (isReservationExpired({ ...session, reservationExpiresAt })) {
            await Promise.all([
              sessionRef.set({ status: "expired", reservationExpiresAt, expiredAt: timestamp, updatedAt: timestamp }, { merge: true }),
              mappingRef.set({ status: "expired", reservationExpiresAt, expiredAt: timestamp, updatedAt: timestamp }, { merge: true }),
              upsertCartLifecycle({ cartOwnerId: safeCartOwnerId, cartId: safeCartId, status: "active", extra: { checkoutSessionId: existingSessionId, reservationStatus: "expired" } }),
            ]);
            return {
              sessionId: existingSessionId,
  ...
  const sessionPayload = {
    ...
    updatedAt: timestamp,
    lastSeenAt: timestamp,
    reservationExpiresAt: cartReservationExpiresAt || reservationExpiresAtFrom(timestamp),
  };
...
export async function resolveCheckoutSession({ sessionId, cartOwnerId, cartId = "" }) {
  ...
  if (isReservationExpired({ ...session, reservationExpiresAt })) {
    await Promise.all([
      sessionRef.set({ status: "expired", reservationExpiresAt, expiredAt: timestamp, updatedAt: timestamp }, { merge: true }),
      upsertCartLifecycle({ cartOwnerId: safeCartOwnerId, cartId: toStr(session?.cartId), status: "active", extra: { checkoutSessionId: safeSessionId, reservationStatus: "expired" } }),
    ]);
    return {
