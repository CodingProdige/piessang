...
export async function listSellerNotifications({ sellerCode = "", sellerSlug = "" } = {}) {
  ...
  const [codeSnap, slugSnap] = await Promise.all([
    code ? db.collection(SELLER_NOTIFICATIONS_COLLECTION).where("sellerCode", "==", code).get() : null,
    slug ? db.collection(SELLER_NOTIFICATIONS_COLLECTION).where("sellerSlug", "==", slug).get() : null,
  ]);
