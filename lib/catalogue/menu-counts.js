...
import { isProductEligibleForShopperCountryServer } from "@/lib/shipping/shopper-country";
...
function productMatchesDefaultListingState(data) {
  return data?.placement?.isActive === true;
}

function applySellerListingData(data, sellerOwner) {
  ...
  return {
    ...
    seller: {
      ...
      sellerSlug: toStr(seller?.sellerSlug || seller?.activeSellerSlug || seller?.groupSellerSlug || data?.seller?.sellerSlug),
      activeSellerSlug: toStr(seller?.activeSellerSlug || seller?.sellerSlug || data?.seller?.activeSellerSlug),
      groupSellerSlug: toStr(seller?.groupSellerSlug || seller?.sellerSlug || data?.seller?.groupSellerSlug),
      deliveryProfile,
      courierProfile,
    },
    product: {
...
export async function computeCatalogueMenuCounts(db, shopperArea = "") {
  ...
  for (const data of products) {
    ...
    const productWithSellerListingData = applySellerListingData(data, sellerOwner);
    if (productMissingSellerDeliverySettings(productWithSellerListingData, sellerOwner)) continue;
    if (!productMatchesDefaultListingState(productWithSellerListingData)) continue;
    if (!productHasListableAvailability(productWithSellerListingData)) continue;
    if (!(await isProductEligibleForShopperCountryServer({ data: productWithSellerListingData }, shopperArea))) continue;
...
export async function rebuildCatalogueMenuCounts(db) {
  const { categoryDocs, subCategoryDocs, categoryCounts, subCategoryCounts } = await computeCatalogueMenuCounts(db);
