...
export async function refreshLiveCommerceSnapshot() {
  ...
  const allPaidOrders = [];
  const liveViewerSessions = new Set();

  for (const docSnap of liveViewsSnap.docs) {
    const value = docSnap.data() || {};
    const sessions = value?.sessions && typeof value.sessions === "object" ? value.sessions : {};
    for (const [sessionId, seenAt] of Object.entries(sessions)) {
      if (!toStr(sessionId)) continue;
      const seenAtMs = parseDateMillis(seenAt);
      if (!seenAtMs) continue;
      liveViewerSessions.add(String(sessionId).trim());
    }
  }

  const viewerCountRightNow = liveViewerSessions.size;
