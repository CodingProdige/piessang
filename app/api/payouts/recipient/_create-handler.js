...
export async function handleCreateRecipient(req) {
  try {
    const db = getAdminDb();
    ...
    return ok({
      ...result,
      provider: "wise",
      message: result?.message || "Wise payout recipient created successfully.",
    });
  } catch (e) {
    const details =
      typeof e?.payload === "object" && e?.payload
        ? JSON.stringify(e.payload).slice(0, 3000)
        : String(e?.message || "").slice(0, 3000);
    return err(e?.status || 500, "Payout Setup Failed", e?.message || "Unable to create the Wise payout recipient.", {
      details,
    });
  }
}
