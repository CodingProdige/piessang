...
function extractVolume(text) {
  ...
  const match = source.match(/(\d+(?:[.,]\d+)?)\s*(kg|g|ml|lt|l|liter|litre|liters|litres)\b/i);
  if (!match) return "";

  const amount = match[1].replace(",", ".");
  const unit = match[2].toLowerCase();

  if (unit === "kg") return `${Math.round(Number.parseFloat(amount) * 1000)}G`;
  if (unit === "g") return `${Math.round(Number.parseFloat(amount))}G`;
  if (unit === "ml") return `${Math.round(Number.parseFloat(amount))}ML`;

  const ml = Number.parseFloat(amount) * 1000;
  if (!Number.isFinite(ml) || ml <= 0) return "";
  return ml % 1000 === 0 ? `${ml / 1000}L` : `${Math.round(ml)}ML`;
}

function inferType(title, variantLabel) {
  const text = `${title} ${variantLabel}`.toLowerCase();
  const types = [
    ["can", "CAN"],
    ["cans", "CAN"],
    ["glass", "GLASS"],
...
function inferFlavor(title, variantLabel) {
  const text = `${title} ${variantLabel}`.toLowerCase();
  const flavors = [
    ["no sugar", "ZERO"],
    ["sugar free", "ZERO"],
...
function inferUnits(text) {
  ...
  const packMatch =
    source.match(/(\d+)\s*(?:x|×|\-?\s*pack|\s*pk|\s*crate|\s*case)\b/i) ||
    source.match(/(?:^|\s)(\d{1,3})(?:\s*pack|\s*pk|\s*crate|\s*case)\b/i);
  if (packMatch) return String(Math.max(1, Number.parseInt(packMatch[1], 10) || 1));

  if (/single/i.test(source)) return "1";
...
function cleanProductTokens(title) {
  const stop = new Set([
    ...
    "kg", "g", "each", "small", "medium", "large", "zero", "original", "regular", "classic", "diet",
...
function buildBaseSku({ brand, title, variantLabel }) {
  const prefix = slugifyPart(brand || "BEVGO");
  const sourceTitle = [title, variantLabel].filter(Boolean).join(" ");
  const productPart = cleanProductTokens(sourceTitle).map(slugifyPart).join("-");
  const volumePart = extractVolume(sourceTitle);
  const typePart = inferType(title, variantLabel);
  const flavorPart = inferFlavor(title, variantLabel);
  const unitsPart = inferUnits(sourceTitle);
...
async function collectUsedSkus(db) {
...
export async function POST(req) {
  try {
    ...
    const used = await collectUsedSkus(db);
    const base = buildBaseSku({ brand, title, variantLabel });
    ...
    console.error("products/variants/skuGenerator failed:", e);
