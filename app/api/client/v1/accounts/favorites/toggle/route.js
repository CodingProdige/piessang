...
export async function POST(req) {
  try {
    ...
    if (!uid || !uniqueId) {
      return err(400, "Missing Fields", "uid and unique_id are required.");
    }
    ...
    const updated = isFavorite
      ? normalizedFavorites.filter(item => item !== uniqueId)
      : [...normalizedFavorites, uniqueId];

    await userRef.update({
      "preferences.favoriteProducts": updated
    });

    return ok({ favorites: updated, isFavorite: !isFavorite });
  } catch (e) {
    return err(500, "Favorites Toggle Failed", e?.message || "Unexpected error.");
  }
}
