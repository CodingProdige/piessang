...
type SellerAnalyticsWorkspaceProps = {
  sellerSlug: string;
  sellerCode: string;
  vendorName: string;
};
...
export function SellerAnalyticsWorkspace({ sellerSlug, sellerCode, vendorName }: SellerAnalyticsWorkspaceProps) {
  const [timeframe, setTimeframe] = useState<TimeframeKey>("30d");
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState<string | null>(null);
  const [orders, setOrders] = useState<SellerOrderSlice[]>([]);
  const [customers, setCustomers] = useState<SellerCustomerSummary | null>(null);
  const [returns, setReturns] = useState<SellerReturnEntry[]>([]);
  const [products, setProducts] = useState<SellerProductEntry[]>([]);
  const [engagement, setEngagement] = useState<{ current: SellerEngagementSummary | null; previous: SellerEngagementSummary | null }>({
    current: null,
    previous: null,
  });
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      setError(null);
      try {
        const sellerQuery = sellerCode ? `sellerCode=${encodeURIComponent(sellerCode)}` : `sellerSlug=${encodeURIComponent(sellerSlug)}`;
        const vendorQuery = encodeURIComponent(vendorName);
        const [ordersResponse, customersResponse, returnsResponse, productsResponse, engagementCurrentResponse, engagementPreviousResponse] = await Promise.all([
          fetch(`/api/client/v1/orders/seller/list?${sellerQuery}`, { cache: "no-store" }),
          fetch(`/api/client/v1/seller/customers?vendorName=${vendorQuery}`, { cache: "no-store" }),
          fetch("/api/client/v1/orders/returns/get", {
            method: "POST",
            headers: { "Content-Type": "application/json" },
            cache: "no-store",
            body: JSON.stringify({ sellerCode, sellerSlug }),
          }),
          fetch(`/api/catalogue/v1/products/product/get?limit=all&includeUnavailable=true&vendorName=${vendorQuery}`, {
            cache: "no-store",
          }),
          fetch(`/api/client/v1/accounts/seller/engagement?${sellerQuery}&vendorName=${vendorQuery}&days=${daysForTimeframe(timeframe)}`, { cache: "no-store" }),
          fetch(`/api/client/v1/accounts/seller/engagement?${sellerQuery}&vendorName=${vendorQuery}&days=${daysForTimeframe(timeframe)}&offsetDays=${daysForTimeframe(timeframe)}`, { cache: "no-store" }),
        ]);
        ...
        const engagementCurrentPayload = await engagementCurrentResponse.json().catch(() => ({}));
        const engagementPreviousPayload = await engagementPreviousResponse.json().catch(() => ({}));

        if (!ordersResponse.ok || ordersPayload?.ok === false) {
          throw new Error(ordersPayload?.message || "Unable to load seller analytics.");
        }

        if (!cancelled) {
          setOrders(Array.isArray(ordersPayload?.items) ? ordersPayload.items : []);
          setCustomers((customersPayload?.data || customersPayload) ?? null);
          setReturns(Array.isArray(returnsPayload?.data) ? returnsPayload.data : []);
          setProducts(Array.isArray(productsPayload?.items) ? productsPayload.items : []);
          setEngagement({
            current: (engagementCurrentPayload?.data || engagementCurrentPayload) ?? null,
            previous: (engagementPreviousPayload?.data || engagementPreviousPayload) ?? null,
          });
        }
      } catch (cause) {
        if (!cancelled) {
          setError(cause instanceof Error ? cause.message : "Unable to load seller analytics.");
        }
      } finally {
  ...
  }, [sellerCode, sellerSlug, timeframe, vendorName]);
  ...
  const analytics = useMemo(() => {
    ...
    const summarize = (items: SellerOrderSlice[]) => {
      ...
      const units = items.reduce((sum, item) => sum + toNum(item?.counts?.quantity), 0);
      const delivered = items.filter((item) => item?.deliveryProgress?.isComplete || toStr(item.fulfillmentStatus).toLowerCase() === "delivered").length;
      const overdue = items.filter((item) => item?.fulfilmentDeadline?.overdue === true).length;
      return {
        revenue,
        orders: items.length,
        units,
    ...
    const salesSeries = buildTimeSeries(currentOrders, periodDays, (item) => item.createdAt, (item) => toNum(item?.totals?.totalIncl));
    const orderSeries = buildTimeSeries(currentOrders, periodDays, (item) => item.createdAt, () => 1);

    const channelRevenue = new Map<string, number>();
    const deliveryMix = new Map<string, number>();
    const regionMix = new Map<string, number>();
    const lineMap = new Map<string, { title: string; revenue: number; units: number; orders: number }>();

    currentOrders.forEach((order) => {
      const amount = toNum(order?.totals?.totalIncl);
      const channelLabel = sentenceCase(order.channel || "Online store");
      channelRevenue.set(channelLabel, (channelRevenue.get(channelLabel) || 0) + amount);

      const deliveryLabel = sentenceCase(order?.deliveryOption?.label || order?.deliveryOption?.type || "Unknown");
      deliveryMix.set(deliveryLabel, (deliveryMix.get(deliveryLabel) || 0) + 1);

      const regionLabel = sentenceCase(order?.destination?.province || order?.destination?.city || order?.destination?.label || "Unknown");
      regionMix.set(regionLabel, (regionMix.get(regionLabel) || 0) + 1);

      const lines = [
        ...(Array.isArray(order?.lines?.selfFulfilment) ? order.lines.selfFulfilment : []),
        ...(Array.isArray(order?.lines?.piessangFulfilment) ? order.lines.piessangFulfilment : []),
      ];

      lines.forEach((line) => {
        const title = toStr(line?.product_snapshot?.product?.title || line?.product_snapshot?.name || "Product");
        const revenue = toNum(line?.line_totals?.final_incl || line?.totals?.final_incl);
        const units = Math.max(1, toNum(line?.quantity));
        const key = title.toLowerCase();
        const currentEntry = lineMap.get(key) || { title, revenue: 0, units: 0, orders: 0 };
        currentEntry.revenue += revenue;
        currentEntry.units += units;
        currentEntry.orders += 1;
        lineMap.set(key, currentEntry);
      });
    });

    const topProducts = Array.from(lineMap.values())
    ...
    const lastCustomer = customerList
      .slice()
      .sort((left, right) => new Date(toStr(right.last_order_at)).getTime() - new Date(toStr(left.last_order_at)).getTime())[0] || null;

    const currentReturns = returns.filter((entry) => isWithinDays(entry?.timestamps?.createdAt, periodDays, 0));
    ...
    const heatmap = buildHeatmap(currentOrders, periodDays);
    ...
    return {
      ...
      totalCustomers: toNum(customers?.total_customers),
      ...
      heatmap,
      recentOrders: currentOrders.slice().sort((a, b) => new Date(toStr(b.createdAt)).getTime() - new Date(toStr(a.createdAt)).getTime()).slice(0, 5),
      engagementCurrent: {
        impressions: toNum(engagement.current?.totals?.impressions),
        clicks: toNum(engagement.current?.totals?.clicks),
        hovers: toNum(engagement.current?.totals?.hovers),
        productViews: toNum(engagement.current?.totals?.productViews),
        ctr: toNum(engagement.current?.totals?.ctr),
      },
      engagementPrevious: {
        impressions: toNum(engagement.previous?.totals?.impressions),
        clicks: toNum(engagement.previous?.totals?.clicks),
        hovers: toNum(engagement.previous?.totals?.hovers),
        productViews: toNum(engagement.previous?.totals?.productViews),
        ctr: toNum(engagement.previous?.totals?.ctr),
      },
      topEngagementProducts: Array.isArray(engagement.current?.topProducts) ? engagement.current.topProducts : [],
      engagementSeries: Array.isArray(engagement.current?.daily)
        ? engagement.current.daily.map((entry) => ({
  ...
  }, [customers, engagement.current, engagement.previous, orders, periodDays, products, returns]);

  const revenueDelta = compareMoney(analytics.current.revenue, analytics.previous.revenue);
  const ordersDelta = compareNumber(analytics.current.orders, analytics.previous.orders);
  const averageDelta = compareMoney(analytics.current.avgOrder, analytics.previous.avgOrder);
  const unitsDelta = compareNumber(analytics.current.units, analytics.previous.units);
  const viewsDelta = compareNumber(analytics.engagementCurrent.productViews, analytics.engagementPrevious.productViews);
  const clicksDelta = compareNumber(analytics.engagementCurrent.clicks, analytics.engagementPrevious.clicks);
  const ctrDelta = compareNumber(analytics.engagementCurrent.ctr, analytics.engagementPrevious.ctr);
  const periodLabel = timeframe === "7d" ? "Last 7 days" : timeframe === "30d" ? "Last 30 days" : "Last 90 days";
  const nextPayoutLabel = analytics.current.overdue
    ? `${analytics.current.overdue} overdue fulfilment item${analytics.current.overdue === 1 ? "" : "s"}`
    : analytics.current.delivered > 0
      ? `${analytics.current.delivered} delivered order${analytics.current.delivered === 1 ? "" : "s"}`
      : "No deliveries yet";

  const deliveryDonut = analytics.deliveryMix.map((entry, index) => ({
    ...entry,
  ...
  function exportBoardSummary() {
    const lines = [
      `${vendorName || "Seller"} analytics`,
      `Timeframe: ${periodLabel}`,
      `Gross sales: ${formatMoney(analytics.current.revenue)}`,
      `Orders: ${analytics.current.orders}`,
      `Average order: ${formatMoney(analytics.current.avgOrder)}`,
      `Items sold: ${analytics.current.units}`,
    ];
    void navigator.clipboard.writeText(lines.join("\n")).then(() => setCopiedMessage("Analytics summary copied."));
  }

  function shareRevenueSnapshot() {
    const text = `${vendorName || "Seller"} • ${periodLabel} • ${formatMoney(analytics.current.revenue)} from ${analytics.current.orders} orders`;
      ...
      <section className="overflow-hidden rounded-[24px] border border-black/6 bg-[linear-gradient(180deg,#ffffff_0%,#f7fbff_100%)] p-5 shadow-[0_14px_34px_rgba(20,24,27,0.06)]">
        <div className="flex flex-col gap-4 xl:flex-row xl:items-end xl:justify-between">
          <div>
            ...
            <h2 className="mt-2 text-[24px] font-semibold tracking-[-0.04em] text-[#202020]">{vendorName || "Your store"} performance</h2>
          ...
          <section className="columns-1 gap-4 md:columns-2 xl:columns-3">
              ...
              >
                <div className="space-y-3">
                  ...
                  <div className="grid gap-3 sm:grid-cols-3">
                    ...
                    <div className="rounded-[18px] border border-black/6 bg-white p-4 shadow-[0_8px_18px_rgba(20,24,27,0.04)]">
                      <p className="text-[11px] font-semibold uppercase tracking-[0.12em] text-[#8b94a3]">Store health</p>
            ...
            <div className="mb-4 [break-inside:avoid]">
              ...
              >
                <div className="space-y-4">
                  <HorizontalBars
                    items={analytics.channelRevenue.map((entry) => ({
                      label: entry.label,
                      value: entry.value,
                ...
                <div className="space-y-3">
                  ...
                  {analytics.topEngagementProducts.length ? (
                    <div className="space-y-2">
                      {analytics.topEngagementProducts.slice(0, 5).map((product) => (
            ...
            <div className="mb-4 [break-inside:avoid]">
              ...
              >
                <HorizontalBars
                  items={analytics.regionMix.map((entry) => ({ label: entry.label, value: entry.value }))}
                  ...
                  {analytics.recentOrders.length ? analytics.recentOrders.map((order) => (
                    <div key={`${order.orderId}-${order.orderNumber || "order"}`} className="rounded-[16px] border border-black/6 bg-[#fafafa] px-4 py-3">
                      <div className="flex items-start justify-between gap-3">
                        ...
                        <span className="text-[13px] font-semibold text-[#202020]">{formatMoney(toNum(order?.totals?.totalIncl))}</span>
                      </div>
                    </div>
                  )) : (
                    <div className="rounded-[18px] border border-dashed border-black/10 bg-[#fafafa] px-4 py-6 text-[13px] text-[#7a8594]">
...
export default SellerAnalyticsWorkspace;
